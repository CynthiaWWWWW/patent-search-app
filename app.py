...
# --- 3. 終極 CSS 優化 (加大標籤 + 雙語對照 + 一頁式佈局) ---
st.markdown("""
    <style>
    /* 極致壓縮邊距 */
    .block-container { padding-top: 1rem !important; padding-bottom: 0rem !important; }
    
    .main-title { font-size: 24px !important; font-weight: 800; color: #1E1E1E; margin-bottom: 5px; text-align: center; }
    ...
    .label-en-sub { font-size: 12px; color: #777; font-weight: 400; margin-left: 4px; }
    ...
    .stLinkButton > a {
        ...
        box-shadow: 0 8px 15px rgba(66, 133, 244, 0.2) !important;
    }
    .stLinkButton > a:hover {
        transform: translateY(-2px) !important;
        box-shadow: 0 12px 20px rgba(66, 133, 244, 0.3) !important;
    }
...
with st.container(border=True):
    ...
    with col1:
        # 關鍵字區
        st.markdown('<span class="field-label">技術關鍵字 <span class="label-en-sub">Technology Keywords</span></span>', unsafe_allow_html=True)
        ...
        # 專利權人區
        st.markdown('<span class="field-label">專利權人 (公司) <span class="label-en-sub">Assignee / Company</span></span>', unsafe_allow_html=True)
        assignee = st.text_input("公司", placeholder="例如: Medtronic", key="k3", label_visibility="collapsed")
        exact_as = st.checkbox("公司精準比對", value=True, key="ex_as")
    
    with col2:
        # 發明人與 CPC 區
        st.markdown('<span class="field-label">發明人與分類 <span class="label-en-sub">Inventor & CPC</span></span>', unsafe_allow_html=True)
        ...
        exact_iv = st.checkbox("發明人精準比對", value=True, key="ex_iv")
        
        # 顯示設定區
        st.markdown('<span class="field-label">每頁顯示筆數 <span class="label-en-sub">Results Per Page</span></span>', unsafe_allow_html=True)
...
if params:
    ...
    # 合併大按鈕
    st.link_button(f"🚀 確認條件並前往 Google Patents", google_url, use_container_width=True)

else:
    st.info("💡 請於上方填寫檢索條件以生成指令。")
